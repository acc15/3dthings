...
function bl_arc_steps(r, d, a) = let(aa = bl_angle_cast(a)) $fn > 0 ? $fn : ceil(max(min((aa[1] - aa[0]) / $fa, bl_ellipse_perimeter(r, d) / $fs), 5));
...
function bl_arc_loop(r, d, a, n, l = true, p = [0,0]) =
    let(ra = bl_radius_cast(r, d), aa = bl_angle_cast(a)) ra[0] == 0 || ra[1] == 0 
        ? [p] 
        : [ for(i = [0 : last ? n : n - 1]) bl_arc_pt(ra, undef, aa[0] + i * (aa[1] - aa[0]) / n, p) ];

function bl_arc(r, d, a, l = true, p = [0,0]) = bl_arc_loop(r, d, a, bl_arc_steps(r, d, a), l, p);

/** Generates NGON shape */
function bl_ngon(r, d, n, p = [0,0]) = bl_arc_loop(r, d, [0, 360], n, false, p);
